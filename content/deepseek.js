...
function insertDiffPanel(fragment) {
  var containers = document.querySelectorAll(
    '[class*="ds-markdown"], [class*="chat-message"], [class*="assistant"]'
  );
  var target = containers.length > 0 ? containers[containers.length - 1] : null;
  if (!target) {
    var scroll = document.querySelector('#chat-container, main');
    if (scroll && scroll.lastElementChild) { target = scroll.lastElementChild; }
  }
  if (target) {
